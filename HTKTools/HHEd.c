...
{
   Source src;
   char qname[256],buf[2048],info[256];
   char fn[256];
...
/*chaopig*/
printf("before strcpy, kwd = %s\n", kwd);
        strcpy(buf, kwd);
/*chaopig*/
printf("after strcpy, kwd = %s\n", buf);
    }
/*chaopig*/
printf("AA\n");
printf("buf 0 = %s\n", buf);
...
/*chaopig*/
printf("InsertOneANNLayer 1: INPUTFEA, buf = %s\n", buf);
        ChkedAlpha("Next keyword", buf);
/*chaopig*/
printf("InsertOneANNLayer 2: INPUTFEA, buf = %s\n", buf);
        if (strcmp(buf, "~e") == 0) {
...
static char *cmdmap[] = {"AT","RT","SS","CL","CO","JO","MU","TI","UF","NC",
                         ...
                         "AF", "IL", "CF", "CD", "EL", "CA", "EF", "LR", "CP", "DL", "CM", "RS", "CH", "" };
