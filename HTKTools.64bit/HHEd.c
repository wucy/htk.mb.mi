...
/* cz277 - ANN */
/*#define RELUSCALE    8.0*/
/*#define RELUSCALE    16.0*/
/*#define RELUSCALE    256.0*/
#define RELUSCALE    64.0
...
/* cz277 - ANN */
/*static HMMSet *auxset;*/	   /* the HMMSet used for auxiliary purpose */
...
void ConcatenateCommand(void)
{
   ...
   for (i=1; i<=n; i++) {
      ...
      if(macroName == NULL) {
         HError(999,"ConcatenateCommand: Unknown label %s", Id->name);
      }
...
static ADLink GetANNDefByMacName(char *invoker, char *kwd, HMMSet *curset) {
    ...
    if (id != NULL && (m = FindMacroName(curset, 'n', id)) != NULL) {
        ...
        HError(9999, "%s: Cannot find target ANN model", invoker);
    }
...
static LELink GetLayerElemByMacName(char *invoker, char *kwd, HMMSet *curset, ADLink *owner, int *pos) {
    ...
    if (strcmp(buf, "~n") == 0) {
        annDef = GetANNDefByMacName(invoker, buf, curset);
        i = ChkedInt("Target ANN layer index", 2, annDef->layerNum + 1) - 2;
        layerElem = annDef->layerList[i];
    }
    else if (strcmp(buf, "~k") == 0) {
        ChkedAlpha("ANN layer macro name", buf);
        id = GetLabId(buf, FALSE);
        if (id != NULL && (m = FindMacroName(curset, 'k', id)) != NULL) {
            layerElem = (LELink) m->structure;
        }
        else {
            HError(9999, "%s: Cannot find the target ANN layer", invoker);
        }
        ...
        HError(9999, "%s: Unsupported target ANN layer macro type", invoker);
    }
    if (owner != NULL)
        *owner = annDef;
    if (pos != NULL)
...
static FELink ParseNextFeaElem(char *invoker, char *kwd, int *feaIdx, int *feaDim) {
    ...
    if (strcmp(buf, "<FEATURE>") != 0) {
        HError(9999, "%s: <FEATURE> keyword expected", invoker);
    }
    ...
    if (strcmp(buf, "<SOURCE>") == 0) {
        ...
        if (strcmp(buf, "<AUGFEA>") == 0) {
            ...
            if (hset->pkind != Str2ParmKind(buf)) {
                HError(9999, "%s: Input parmkind different from the input parmKind to the model", invoker);
            }
            feaElem->inputKind = INPFEAIK;
            feaElem->feaSrc = NULL;
            streamIdx = 1;
        ...
        HError(9999, "%s: <SOURCE> keyword expected", invoker);
    }
    if (strcmp(buf, "<DIMRANGE>") == 0) { 
        feaElem->dimOff = ChkedInt("Start dimension", 1, 65535) - 1;
        intVal = ChkedInt("End dimension", 2, 65535);
        if (feaElem->feaDim != intVal - feaElem->dimOff) {
            HError(9999, "%s: Inconsistent feature element dimension range", invoker);
        }
        ChkedAlpha("Next keyword", buf);
    }
    if (strcmp(buf, "<EXPAND>") == 0) {
        ...
        for (i = 1; i <= intVal; ++i) {
            feaElem->ctxMap[i] = ChkedInt("Context expansion offset", -INT_MAX, INT_MAX);
        }
        if (feaElem->inputKind == ANNFEAIK) {
            /* TODO: check illegal context expansion for ANNFEAIK */
        }
        /*if (feaElem->inputKind != INPFEAIK) {
            HError(9999, "%s: Context expansion for this feature element type is not supported", invoker);
        }*/
    }
    else {
        HError(9999, "%s: <EXPAND> keyword expected", invoker);
    }
    ...
    if (feaElem->inputKind == INPFEAIK || feaElem->inputKind == AUGFEAIK) {
        ...
        if (i == hset->nInp[streamIdx]) {
            if (i >= MAXINPUSE) {
                HError(9999, "%s: Maximum number of input feature usage reached", invoker);
            }
...
/* cz277 - 1007 */
FeaMix *AddOneFeaMix(char *invoker, char *kwd) {
    ...
    if (strcmp(buf, "<MACNAME>") == 0) {
        ...
        id = GetLabId(buf, FALSE);
        if (id != NULL) {
            m = FindMacroName(hset, 'e', id);
            feaMix = (FeaMix *) m->structure;
            if (feaMix->nUse != 0) {
                HError(9999, "%s: specified layer macro name occupied", invoker);
            }
        }
        else {
            id = GetLabId(buf, TRUE);
        ...
        id = GetNextANNMacroName(invoker, hset, 'e');
        printf("%s: Add a feature mixture with automatically assigned macro name %s\n", invoker, id->name);
    }
    if (strcmp(buf, "<NUMFEAS>") == 0) {
        ...
        HError(9999, "%s: Incorrect keyword for input feature", invoker);
    }
    /* reset owner num */
    feaMix->ownerNum = 0;
    /* add a new macro */
    if (m == NULL) {
        NewMacro(hset, fidx, 'e', id, feaMix);
    }
    else {
...
void InsertOneANNLayer(void) {
    int i, j, k, layerPos, layerNum, nodeNum, inputDim;
    int intVal, feaDim, feaIdx;
    ADLink annDef;
    AILink annInfo;
    LELink layerElem;
    LELink *layerList;
    ...
    if (strcmp(buf, "~n") != 0) {
        HError(9999, "InsertOneANNLayer: ~n macro type expected");
    }
    ChkedAlpha("Target ANN macro name", buf);
    ...
    if (layerPos >= 0 && layerPos < 2)
        HError(9999, "InsertOneANNLayer: Illegal insert layer position");
    else if (layerPos >= 2)
        layerPos -= 2;
    else
        layerPos = annDef->layerNum + 1 + layerPos;
    /* get the dimensions of the new layer and insert it */
    nodeNum = ChkedInt("Node number of the new layer", 1, INT_MAX);
    inputDim = ChkedInt("Input dimension of the new layer", 1, INT_MAX);
    layerElem = GenNewLayer(hset->hmem, nodeNum, inputDim);
    ChkedAlpha("Keyword token", buf); 
    if (strcmp(buf, "<MACNAME>") == 0) {	/* optional */
        ChkedAlpha("Layer macro type", buf);
        ChkedAlpha("Layer macro name", name);
        id = GetLabId(name, FALSE);
        if (id != NULL) {
            HError(9999, "InsertOneANNLayer: specified layer macro name occupied");
        }
        ChkedAlpha("Keyword token", buf);
    }
    else {
        id = GetNextANNMacroName("InsertOneANNLayer", hset, 'k');
        printf("Insert a layer with automatically assigned macro name %s\n", id->name);
    }
    ++layerElem->nUse;
    id = GetLabId(name, TRUE);
    NewMacro(hset, fidx, 'k', id, layerElem);
    /* <OPERAND> */
    if (strcmp(buf, "<OPERAND>") == 0) {
        ChkedAlpha("Operation kind of the new layer", buf);
    ...
    /* <INPUTFEA> */
    if (strcmp(buf, "<INPUTFEA>") == 0) {
        ChkedAlpha("Next keyword", buf);
        if (strcmp(buf, "~e") == 0) {
            ChkedAlpha("Feature mixture macro name", buf);
            id = GetLabId(buf, FALSE);
            if (id != NULL && (m = FindMacroName(hset, 'e', id)) != NULL) {
                layerElem->feaMix = (FeaMix *) m->structure;
            }
            else {
                HError(9999, "InsertOneANNLayer: Unexisted feature mixture macro");
            }
            /* set nDrv */
            ...
            layerElem->feaMix = AddOneFeaMix("InsertOneANNLayer", buf);
        }
        /* set owner and nUse properly */
        ...
        HError(9999, "InsertOneANNLayer: <INPUTFEA> expected");
    }
...
static void ChangeOneLayerDims(LELink layerElem, int seed, int nodeNum, int inputDim) {
    NMatrix *wghtMat;
    NVector *biasVec;
    float randScale = 1.0;

    wghtMat = layerElem->wghtMat;
    biasVec = layerElem->biasVec;
    layerElem->wghtMat = CreateNMatrix(hset->hmem, nodeNum, inputDim);
    layerElem->biasVec = CreateNVector(hset->hmem, nodeNum);
    layerElem->nodeNum = nodeNum;
    layerElem->inputDim = inputDim;
    if (layerElem->actfunKind == RELAF || layerElem->actfunKind == SOFTRELAF) {
        randScale = 1.0 / RELUSCALE;
    }
    RandANNLayer(layerElem, seed, randScale);
    CopyPartialNMatrix2NMatrix(wghtMat, layerElem->wghtMat);
    CopyPartialNVector2NVector(biasVec, layerElem->biasVec);
}
...
void ChangeOneANNLayerInput(void) {
    ...
    ChkedAlpha("Target layer macro type", buf);
    if (strcmp(buf, "~n") == 0 || strcmp(buf, "~k") == 0) {
        layerElem = GetLayerElemByMacName("ChangeOneANNLayerInput", buf, hset, NULL, NULL);
    }
    else {
        HError(9999, "ChangeOneANNLayerInput: Unsupported target ANN layer macro");
    }
    ...
    if (strcmp(buf, "~e") == 0) {
        ...
        feaMix = AddOneFeaMix("ChangeOneANNLayerInput", buf);
    }
    /* deuse the old feature mixture */
    /*for (i = 0, j = 0; j < layerElem->feaMix->ownerNum; ++i, ++j) {
        if (layerElem->feaMix->ownerList[i] == layerElem) 
...
void ChangeOneANNLayerDims(void) {
    ...
    if (nodeNum != layerElem->nodeNum || inputDim != layerElem->inputDim)
        ChangeOneLayerDims(layerElem, layerPos, nodeNum, inputDim);
    /* saves the model */
    saveHMMSet = TRUE;
}
...
void EraseOneANNLayer(void) {
    ...
    if (strcmp(buf, "~n") == 0 || strcmp(buf, "~k") == 0) {
        ...
        HError(9999, "EraseOneANNLayer: Unsupported target ANN layer macro");
    }
    /* take action */
    actfunKind = layerElem->actfunKind;
    if (actfunKind == RELAF || actfunKind == SOFTRELAF) {
        randScale = 1.0 / RELUSCALE;
    }
    else {
        randScale = 1.0;
...
void ChangeOneANNLayerActFun(void) {
    ...
    if (strcmp(buf, "<ACTIVATION>") == 0) {
        ...
        HError(9999, "ChangeOneANNLayerActFun: <ACTIVATION> keyword expected");
    } 
...
static FeaMix *CloneOneFeaMix(FeaMix *srcMix) {
    ...
    for (i = 0; i < srcMix->elemNum; ++i) {
        ...
        dstElem->feaMat = CreateNMatrix(hset->hmem, GetNBatchSamples(), dstElem->extDim);;
...
void ExtendOneFeaMix(void) {
    ...
    if (strcmp(buf, "~n") == 0 || strcmp(buf, "~k") == 0) {
        ...
        if (layerElem->feaMix->nUse > 1) {	/* if shared */
            ...
            id = GetLabId(buf, TRUE);*/
            id = GetNextANNMacroName("ExtendOneFeaMix", hset, 'e');
            NewMacro(hset, fidx, 'e', id, feaMix);
            /*++hset->eTypeMacNum;*/
            /* set feaMix back to layerElem->feaMix */
            layerElem->feaMix = feaMix;
        }
        else {
            feaMix = layerElem->feaMix;
        ...
        if (id != NULL && (m = FindMacroName(hset, 'e', id)) != NULL) {
            ...
            HError(9999, "ExtendOneFeaMix: Unexisted feature mixture macro");
        } 
    }
    else {
        HError(9999, "ExtendOneFeaMix: Unsupported target ANN layer macro");
    }
    ...
    if (incIdx > 0 && incIdx <= feaMix->elemNum) {
        HError(9999, "ExtendOneFeaMix: Unacceptable new feature element index, please use -1 instead");
    }
    ...
    for (i = 0; i < feaMix->ownerNum; ++i) {
        ...
        if (layerPos < 0)
            ...
            seed = layerPos;
        ChangeOneLayerDims(layerElem, seed, layerElem->nodeNum, feaMix->mixDim);
    }
    /* saves the model */
    saveHMMSet = TRUE;
}
...
/* LR */
void LowRankApproxOneANNLayer(void) {
    char buf[MAXSTRLEN];
    ADLink annDef;
    LELink origLayer, lowLayer, highLayer, *layerList;
    int i, j, layerPos, layerNum, bnDim, inputDim, nodeNum;
    LabId id;
    NMatrix *A, *U, *V;
    NVector *d;
    NFloat sumS, curS;
    float percent, randScale = 1.0;

    origLayer = GetLayerElemByMacName("LowRankApproxOneANNLayer", NULL, hset, &annDef, &layerPos);

    /* do low rank approximate */
    nodeNum = origLayer->nodeNum;
    inputDim = origLayer->inputDim;
#ifdef MKL
    A = origLayer->wghtMat;
    U = CreateNMatrix(&hmmHeap, nodeNum, nodeNum);
    d = CreateNVector(&hmmHeap, inputDim);
    V = CreateNMatrix(&hmmHeap, inputDim, inputDim);
    NanySVD(A, U, d, V);
    /* scale U with d */
    for (i = 0; i < nodeNum; ++i)
        for (j = 0; j < inputDim; ++j) 
            U->matElems[i * U->colNum + j] *= d->vecElems[j];
#endif

/*#ifdef CUDA
    SyncNMatrixHost2Dev(U);
    SyncNVectorHost2Dev(d);
    SyncNMatrixHost2Dev(V);
#endif*/
    
    /* get the right bottleneck dim, if go percent */
    percent = ChkedFloat("Bottleneck dimension control parameter", 0.0, MIN(origLayer->inputDim, origLayer->nodeNum) + 1);
    if (percent <= 1.0) {       /* pertentage */
#ifndef MKL
        HError(9999, "LowRankApproxOneANNLayer: percent is only available to MKL compiled program");
#endif
        if (percent == 0.0)
            HError(9999, "LowRankApproxOneANNLayer: percent should be bigger 0");
        bnDim = 0;
    }
    else {      /* bottleneck dimension */
        bnDim = (int) (percent + 0.5);
        percent = 0;
    }
#ifdef MKL
    if (bnDim == 0) {
        for (i = 0, sumS = 0.0; i < inputDim; ++i)
            sumS += d->vecElems[i];
	for (i = 0, curS = 0.0; i < inputDim; ++i) {
            curS += d->vecElems[bnDim++];
            if (curS / sumS >= percent)
                break;
        }        
    }
#endif
    ...
    /* generate lowLayer */
    lowLayer = GenNewLayer(hset->hmem, bnDim, inputDim);
    lowLayer->ownerHead = origLayer->ownerHead;
    lowLayer->ownerTail = origLayer->ownerTail;
    ++lowLayer->ownerCnt;
    lowLayer->operKind = SUMOK;
    lowLayer->actfunKind = LINEARAF;
    lowLayer->feaMix = origLayer->feaMix;
    for (i = 0; i < lowLayer->feaMix->ownerNum; ++i) {
        if (lowLayer->feaMix->ownerList[i] == origLayer) {
            lowLayer->feaMix->ownerList[i] = lowLayer;
            break;
        }
    }
    ++lowLayer->nUse;
    /*    regist a new macro */
    id = GetNextANNMacroName("LowRankApproxOneANNLayer", hset, 'k');
    NewMacro(hset, fidx, 'k', id, lowLayer);
    /*    clear orig bias vector */
    ClearNVector(lowLayer->biasVec);

    /* change origLayer to highLayer */
    highLayer = origLayer;
    highLayer->inputDim = bnDim;
    highLayer->wghtMat = CreateNMatrix(&hmmHeap, nodeNum, bnDim);
    highLayer->feaMix = GetDefaultANNFeaMix(hset, lowLayer);
    highLayer->feaMix->ownerList[highLayer->feaMix->ownerNum++] = highLayer;
    ++highLayer->feaMix->nUse;

#ifdef MKL
    /* copy U and Vt */
    CopyPartialNMatrix2NMatrix(V, lowLayer->wghtMat);
    CopyPartialNMatrix2NMatrix(U, highLayer->wghtMat);
#else
    HError(-1, "LowRankApproxOneANNLayer: For non-MKL program, new layers are randomly initialised");
    if (highLayer->actfunKind == RELAF || highLayer->actfunKind == SOFTRELAF) {
        randScale /= RELUSCALE;
    }
    RandANNLayer(lowLayer, layerPos, randScale);
    RandANNLayer(highLayer, layerPos + 1, randScale);
#endif

    /* insert the new layers */
    ++annDef->layerNum;
    layerList = (LELink *) New(hset->hmem, annDef->layerNum * sizeof(LELink));
    for (i = 0, j = 0; i < annDef->layerNum; ++i) {
        if (i == layerPos) {
            layerList[i] = lowLayer;
        }
        else {
            layerList[i] = annDef->layerList[j++];
        }
...
#ifdef MKL
    FreeNVector(&gstack, d);
    FreeNMatrix(&gstack, V);
    FreeNMatrix(&gstack, U);
#endif
    /* saves the model */
    saveHMMSet = TRUE;   
}
...
void CopyANNLayerParms(void) {
    char mmfPath[MAXSTRLEN], lstPath[MAXSTRLEN], buf[MAXSTRLEN];
    ...
    for (i = 0; i < opCnt; ++i) {
        ...
        if (strcmp(buf, "<COPY>") == 0) {
            ...
            opCode = ChkedInt("Operational code", 0, INT_MAX);
            if (srcLayer != dstLayer) {
                /* if ((opCode & ACTFUNUK) != 0) */
                if (opCode == ACTFUNUK) {
                    HError(9999, "CopyANNLayerParms: Unimplement activation parameter copy yet");
                }
                if ((opCode & BIASUK) != 0) {
                    if (srcLayer->inputDim != dstLayer->inputDim) {
                        HError(-1, "CopyANNLayerParms: Unequal bias vector lenghts");
                    }
                    CopyPartialNVector2NVector(srcLayer->biasVec, dstLayer->biasVec);
                }
                if ((opCode & WEIGHTUK) != 0) {
                    ...
                    CopyPartialNMatrix2NMatrix(srcLayer->wghtMat, dstLayer->wghtMat);
                }
            }
        }
        else {
...
void ShowANNLayerStats(void) {
    ...
    if (strcmp(buf, "~n") == 0 || strcmp(buf, "~k") == 0) {
        ...
        HError(9999, "ShowANNLayerStats: Unsupported target ANN layer macro");
    }
    /* compute for weights */
    ...
    l2norm = 0.0;
    for (i = 0; i < len; ++i) {
        absval = fabs(layerElem->wghtMat->matElems[i]);
        if (absval > absmax) 
            absmax = absval;
        mean += layerElem->wghtMat->matElems[i];
        l1norm += absval;
        l2norm += pow(layerElem->wghtMat->matElems[i], 2.0);
    }
    mean /= len;
    l1norm /= layerElem->nodeNum;
    l2norm /= layerElem->nodeNum;
    for (i = 0; i < len; ++i) {
        stddev += pow(layerElem->wghtMat->matElems[i] - mean, 2.0);
    }
    ...
    l2norm = 0.0;
    for (i = 0; i < len; ++i) {
        absval = fabs(layerElem->biasVec->vecElems[i]);
        if (absval > absmax)
            absmax = absval;
        mean += layerElem->biasVec->vecElems[i];
        l1norm += absval;
        l2norm += pow(layerElem->biasVec->vecElems[i], 2.0);
    }
    mean /= len;
    l1norm /= layerElem->nodeNum;
    l2norm /= layerElem->nodeNum;
    for (i = 0; i < len; ++i) {
        stddev += pow(layerElem->biasVec->vecElems[i] - mean, 2.0);
    }
    stddev = sqrt(stddev / len);
    /* output */
    printf("\tbiases:\n");
...
void ReplaceHMMState(void) {
    ...
    if (hset->hsKind == ANNHS) {
        HError(9999, "ReplaceHMMState: No HMMs in the model file");
    }
    ...
    if (hmmId == NULL) {
        HError(9999, "ReplaceHMMState: Failed to find model for label \"%s\"", buf);
    }
    if ((macDef = FindMacroName(hset, 'l', hmmId)) == NULL) {
        HError(9999, "ReplaceHMMState: Unknown HMM name %s", buf);
    }
    ...
    if (stateId == NULL) {
        ...
        if ((macDef = FindMacroName(hset, 'l', stateId)) == NULL) {
            HError(9999, "ReplaceHMMState: Unknown HMM name %s", hbuf);
        }
        si = ((HLink) macDef->structure)->svec[sIdx].info;
    }
    else {
        if ((macDef = FindMacroName(hset, 's', stateId)) == NULL) {
            HError(9999, "ReplaceHMMState: Unknown state name %s", buf);
        }
        ...
        for (i = 1; i <= hset->swidth[0]; ++i) {
            ...
            if (tIdx < layerElem->nodeNum - 1) {
                for (j = tIdx + 1; j < layerElem->nodeNum; ++j) {
                    CopyNSegment(layerElem->wghtMat, j * layerElem->inputDim, layerElem->inputDim, layerElem->wghtMat, (j - 1) * layerElem->inputDim);
                    CopyNVectorSegment(layerElem->biasVec, j, 1, layerElem->biasVec, j - 1);
                }
                --layerElem->nodeNum;
            }
            NewHMMScan(hset, &hss);
...
void ConnectANNtoHMMs(void) {
    ...
    ADLink annDef;
    int i, h, targetCnt, streamIdx = 1;
    LabId id;

    if (hset->hsKind == ANNHS) {
        HError(9999, "ConnectANNtoHMMs: No HMMs found in the model set");
    }
    ...
    AddMMF(auxset, mmfPath);
    if (MakeHMMSet(auxset, lstPath) < SUCCESS)
        HError(9999, "ConnectANNtoHMMs: MakeHMMSet failed");
    if (LoadHMMSet(auxset, NULL, NULL) < SUCCESS)
        HError(9999, "ConnectANNtoHMMs: LoadHMMSet failed");
    /* fetch the ANN model */
    ...
    for (i = 0; i < annDef->layerNum; ++i) {
        layerElem = annDef->layerList[i];
        m = FindMacroStruct(hset, 'k', layerElem);
        if (m == NULL) {
            m = FindMacroStruct(auxset, 'k', layerElem);
            if (m != NULL) {
                id = m->id;
                if (FindMacroName(hset, 'k', id) != NULL)
                    m = NULL;
            }
            if (m == NULL) {
                id = GetNextANNMacroName("ConnectANNtoHMMs", hset, 'k');
            }
            NewMacro(hset, fidx, 'k', id, layerElem);
        }
        m = FindMacroStruct(hset, 'e', layerElem->feaMix);
        if (m == NULL) {
            m = FindMacroStruct(auxset, 'e', layerElem->feaMix);
            if (m != NULL) {
                id = m->id;
                if (FindMacroName(hset, 'e', id) != NULL)
                    m = NULL;
            }
            if (m == NULL) {
                id = GetNextANNMacroName("ConnectANNtoHMMs", hset, 'e');
            }
            NewMacro(hset, fidx, 'e', id, layerElem->feaMix);
        }
    } 
    ...
    if (strcmp(buf, "<HYBRID>") == 0) {
        ...
        if (targetCnt != layerElem->nodeNum) {
            HError(-1, "ConnectANNtoHMMs: Change target number to %d to match HMM state number", targetCnt);
            ChangeOneLayerDims(layerElem, annDef->layerNum - 1, targetCnt, layerElem->inputDim);
        }
        ...
        HError(9999, "ConnectANNtoHMMs: Unknown connection method, <HYBRID> or <TANDEM> expected");
    }
...
static char *cmdmap[] = {"AT","RT","SS","CL","CO","JO","MU","TI","UF","NC",
                         ...
                         "AF", "IL", "CF", "CD", "EL", "CA", "EF", "LR", "CP", "DL", "CM", "RS", "CH", "SL", "" };
...
void DoEdit(char * editFn)
{
   ...
   for (;;) {
      ...
      case AF: AddOneFeaMix("AddOneFeaMix", NULL); break;   	/* done */
      ...
      case LR: LowRankApproxOneANNLayer(); break;		/* done */
      case CP: CopyANNLayerParms(); break;			/* done */
