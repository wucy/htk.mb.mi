...
/*                                                             */
/* ----------------------------------------------------------- */
/*             File: HNCache.c  Cache for ANN models           */
/* ----------------------------------------------------------- */

char *hncache_version = "!HVER!HNCache:   3.4.1 [CUED 30/11/13]";
char *hncache_vc_id = "$Id: HNCache.c,v 1.1.1.1 2013/11/13 09:54:58 cz277 Exp $";
...
static inline void KnuthShuffle(void *list, size_t stPos, size_t edPos, int unitLen) {
    ...
    char buf[256];  /* the temp structure */
...
static inline void QuickNetShuffle(void *list, int stPos, int edPos, int unitLen) {
    ...
    if (maxVal < 1 && maxVal > 0x7fffffff) {
        HError(9999, "QuickNetShuffle: maxVal out of range");
    }
...
static inline void ShuffleSegment(void *list, size_t stPos, size_t edPos, int unitLen) {
    switch (shuffKind) {
        ...
        case QUICKNETSK:
            ...
            break;
        default:
            HError(9999, "ShuffleSegment: Unknown shuffle kind");
    }
}
...
void InitNCache(void)
{
    ...
    if (nParm > 0) {
        ...
        if (GetConfInt(cParm, nParm, "CACHESIZE", &intVal)) {
            defaultCacheSamples = intVal;
        }
        if (GetConfStr(cParm, nParm, "VISITKIND", buf)) {
        ...
        if (GetConfStr(cParm, nParm, "SHUFFKIND", buf)) {
            if (strcmp(buf, "KNUTHFIXED") == 0) {
                ...
                HError(9999, "InitNCache: Unknown shuffle kind");
            }
        }
        if (GetConfStr(cParm, nParm, "RANDSEED", buf)) {
            if (strcmp(buf, "CURTIME") == 0) {
                shuffSeed = time(NULL);
            }
            else {
        ...
        /* cz277 - mtload */
        if (GetConfBool(cParm, nParm, "EXTTHREADLOAD", &boolVal)) {
            extThreadLoad = boolVal;
            /*memset(extThread, 0, sizeof(pthread_t) * SMAX);*/
        }
    }
...
DataCache *CreateCache(MemHeap *heap, FILE *scpFile, int scpCnt, HMMSet *hset, Observation *obs, int streamIdx, size_t cacheSamples, VisitKind visitKind, XFInfo *xfInfo, LabelInfo *labelInfo, Boolean saveUttName) {
    ...
    if (heap->type != CHEAP) {
        HError(9999, "CreateCache: Only CHEAP is supported for cache");
    }
    /* check the correctness of the cache */
    if (obs == NULL) {
        HError(9999, "CreateCache: NULL observation pointer");
    }
    if ((obs->pk & BASEMASK) == DISCRETE || (obs->pk & HASVQ)) {
        HError(9999, "CreateCache: VQ feature is not supported");
    }
    if (streamIdx <= 0 || streamIdx > obs->swidth[0]) {
        HError(9999, "CreateCache: Illegal stream index");
    }
    ...
    if ((labelInfo != NULL) && (labelInfo->labelKind & LABLK) != 0) {
        cache->labVec = CreateIntVec(&gcheap, cache->batchSamples);
        cache->labMat = cache->outLayer->trainInfo->labMat;
        /*if (((HMMSet *) hset)->annSet->mappedTargetNum > 0) {	
            cache->labMatMapSum = cache->outLayer->trainInfo->labMatMapSum;
        }*/
    }
    else {
        cache->labVec = NULL;
...
void ResetCacheHMMSetCfg(DataCache *cache, HMMSet *hset) {
    if (cache->hmmSet != hset) {
        HError(9999, "ResetCacheConfig: New hset address does not equal to previous");
    }
    /* reset hmmSet */
    /* cache->hmmSet = hset; */
    /* reset outLayer */
...
static ReturnStatus LoadOneUtt(DataCache *cache, int dstPos) {
    ...
    char feaBuf[256], spkBuf[256], labBuf[256], hmmBuf[256];
    char fnBuf[256], pathBuf[256], latBuf[256];
    FILE *filePtr;
    ...
    if (!parmBuf) {
        HError(9999, "LoadOneUtt: Open input data failed");
    }
    /* prepare to load the data */
    uttElem = &cache->uttElems[dstPos];
    if (cache->saveUttName) {
        /* cz277 - newstr */
        /*uttElem->uttName = (char *) New(cache->cmem, 256);
        strcpy(uttElem->uttName, feaBuf);*/
    ...
    for (i = 0; i < len; ++i) {
        ...
        for (j = 1; j <= dim; ++j, ++dstPtr) {   /* just copy the data */
            if (isnan(x[j]))
                HError(9999, "LoadOneUtt: %s, frame no. %d, dim %d has nan value", uttElem->uttName, i, j);
            if (isinf(x[j]))
                HError(9999, "LoadOneUtt: %s, frame no. %d, dim %d has inf value", uttElem->uttName, i, j);
            *dstPtr = x[j];
        }
    }
    /* cz277 - aug */
    ...
    if (cache->labelInfo != NULL) {
        ...
        if ((cache->labelInfo->labelKind & LABLK) != 0) { /* load lab files */
            if (cache->labelInfo->labFileMask != NULL) {
                if (!MaskMatch(cache->labelInfo->labFileMask, spkBuf, feaBuf)) {
                    HError(9999, "LoadOneUtt: Mask %s has no match with segment %s", cache->labelInfo->labFileMask, feaBuf);
                }
		...
		lhmmId = hmmId;
                ...
                if (hmmId == NULL) {
                    HError(9999, "LoadOneUtt: Failed to find model for label \"%s\" given in the input MLF file", hmmBuf);
                }
                if ((macDef = FindMacroName(cache->hmmSet, 'l', hmmId)) == NULL) {
                    HError(9999, "LoadOneUtt: Unknown label %s", hmmId->name);
                }
                ...
                if (sIdx == 0) {    /* if it is a phone label */
                    ...
                    for (j = 3; j < hlink->numStates; ++j) {
                        if ((hlink->svec[2].info->pdf[cache->streamIdx].targetSrc != hlink->svec[j].info->pdf[cache->streamIdx].targetSrc) ||
                            ...
                            HError(9999, "LoadOneUtt: Phone label in the label file does not match the state level definition");
                        }
                    }
                    sIdx = 2;
                }
                else if (sIdx <= 1 || sIdx >= hlink->numStates) {
                    HError(9999, "LoadOneUtt: Illegal state index in the label file");
                }
                ...
                if (streamElem->targetSrc != cache->hmmSet->annSet->outLayers[cache->streamIdx]) {
                    HError(9999, "LoadOneUtt: Only one output layer is allowed in one stream");
                }
                ...
                if (stIdx > edIdx) {
                    HError(9999, "LoadOneUtt: Empty segment");
                }
                /* cz277 - trans */
			...
			transcnt = edIdx - stIdx - 1;
                ...
                for (curIdx = stIdx; curIdx < edIdx; ++curIdx, ++i) {
                    if (curIdx != i) {
                        HError(9999, "LoadOneUtt: Discontinuous Utterance");
                    }
                    uttElem->labIdxes[i] = streamElem->targetIdx - 1;   /* targetIdx >= 1, labIdxes >= 0 */
                }
            }
            if (i != uttElem->uttLen) {
                HError(9999, "LoadOneUtt: Feature and Utterance file length do not match");
            }
        }
        ...
        if (((cache->labelInfo->labelKind & LATLK) != 0) && (cache->streamIdx == 1)) { /* load lattice files */
            ...
            if (cache->labelInfo->latFileMask != NULL) {
                if (!MaskMatch(cache->labelInfo->latFileMask, latBuf, feaBuf)) {
                    HError(9999, "LoadOneUtt: Mask %s has no match with segment %s", cache->labelInfo->latFileMask, feaBuf);
                }
            }
            else {
                ...
                for (i = 0; i < cache->labelInfo->nDenLats; ++i) {
                    if (cache->labelInfo->denLatSubDirPat[0]) {
                        if (!MaskMatch(cache->labelInfo->denLatSubDirPat, spkBuf, latBuf)) {
                            HError(9999, "LoadOneUtt: Mask %s has not match with segment %s", cache->labelInfo->denLatSubDirPat, latBuf);
                        }
                    ...
                    if (cache->labelInfo->latMaskDen != NULL) {
                        if (!MaskMatch(cache->labelInfo->latMaskDen, spkBuf, latBuf)) {
                            HError(9999, "LoadOneUtt: Mask %s has not match with segment %s", cache->labelInfo->latMaskDen, latBuf);
                        }
                    ...
                    if (cache->labelInfo->useLLF) {
                        ...
                        if (!filePtr) {
                            HError(9999, "LoadOneUtt: Could not open file %s", fnBuf);
                        }
                ...
                for (i = 0; i < cache->labelInfo->nNumLats; ++i) {
                    if (cache->labelInfo->numLatSubDirPat[0]) {
                        if (!MaskMatch(cache->labelInfo->numLatSubDirPat, spkBuf, latBuf)) {
                            HError(9999, "LoadOneUtt: Mask %s has no match with segment %s", cache->labelInfo->numLatSubDirPat, latBuf);
                        }
                    ...
                    if (cache->labelInfo->latMaskNum != NULL) {
                        if (!MaskMatch(cache->labelInfo->latMaskNum, spkBuf, latBuf)) {
                            HError(9999, "LoadOneUtt: Mask %s has not match with segment %s", cache->labelInfo->latMaskNum, latBuf);
                        }
                        MakeFN(spkBuf, pathBuf, NULL, fnBuf);
                        strcpy(pathBuf, fnBuf);
                    }
...
static int FillCacheSGT(DataCache *cache) {
    ...
    while (cache->nxtUttPos < cache->tUttNum && cache->frmNum < cache->cacheSamples) {
        ...
        if (LoadOneUtt(cache, cache->nxtUttPos++) < SUCCESS) {
            HError(9999, "FillCacheSGT: Load next utterance failed");
        }
        else {
            ++newUttCnt;
        }
...
static void *FillCacheEXT(void *arg) {
    ...
    while (cache->nxtUttPos < cache->tUttNum && cache->frmNum < cache->cacheSamples) {
        /* load the next utterance */
        if (LoadOneUtt(cache, cache->nxtUttPos++) < SUCCESS) {
            HError(9999, "FillCacheEXT: Load next utterance failed");
        }
        else {
            ++newUttCnt;
        }
...
void LoadCacheData(DataCache *cache) {
    ...
    if (extThreadLoad == TRUE) {
        ...
        if (ret != 0) {
            HError(9999, "LoadCacheData: Fail to create the extra thread for cache loading");
        }
    }
    else {
...
static void UnloadOneUtt(DataCache *cache, int dstPos) {
    ...
    if (uttElem->frmUsed != uttElem->uttLen) {
        HError(9999, "UnloadOneUtt: all frames should be used once before unload");
    }
...
void InitCache(DataCache *cache) {
    ...
    switch (cache->visitKind) {
        ...
        case PLUTTVK:
            ...
            break;
        default:
            HError(9999, "InitCache: Unknown visiting order");
            break;
    }
    /* cz277 - mtload */
    if (extThreadLoad == TRUE) {
...
/* copy 1-of-k hard labels to a label batch */
static inline void CopyHardLabel2Batch(int intLab, int tgtDim, NFloat *dstPtr) {
    if (intLab < 0 || intLab >= tgtDim) {
        HError(9999, "CopyHardLabel2Batch: Illegal label index");
    }
    memset(dstPtr, 0, tgtDim * sizeof(NFloat));
    dstPtr[intLab] = 1;
    /* TODO: GPU support */
}
    ...
    for (i = 0; i < nSamples; ++i) {
        ...
        if (cache->frmPtrs[0].uttIdx < 0) {
            ...
            if (cache->orderPtr == cache->tUttNum && frmIdx == uttElem->uttLen - 1) {   /* frmIdx before UpdateFrmPtr() */
                ...
                HError(9999, "FillBatchUTT: Fail to acquire the next frame");
            }
        }
...
static inline int GetDataFromCache(DataCache *cache, FELink feaElem, NFloat *feaBat, int offset) {
...
static inline int GetAugFeaFromCache(DataCache *cache, FELink feaElem, NFloat *feaBat) {
    int i, j, uttIdx;
    NFloat *feaPtr;
    UttElem *uttElem;
    ...
    for (i = 0; i < cache->batLen; ++i) {
        ...
        /* get the right UttElem */
        uttElem = &cache->uttElems[uttIdx];
        /* check the feature dimension and copy the augmented feature vector */
...
/* return the number of frame in the batch */
static inline int GetLabelFromCache(DataCache *cache, int tgtDim, NFloat *labBat, int offset) {
    int i, uttIdx, frmIdx;
    NFloat *labPtr;
    UttElem *uttElem;

    if ((cache->labelInfo->labelKind & LABLK) == 0) {
        HError(9999, "GetLabelFromCache: Function does not support current label kind");
        return -1;
    }
    ...
    for (i = offset; i < cache->batLen; ++i, labPtr += tgtDim) {
        ...
        /* get the right UttElem */
        uttElem = &cache->uttElems[uttIdx];
        cache->labVec[i + 1] = uttElem->labIdxes[frmIdx];   /* fill labVec */
        CopyHardLabel2Batch(uttElem->labIdxes[frmIdx], tgtDim, labPtr);
    }
...
UttElem *GetCurUttElem(DataCache *cache) {
    if (cache->visitKind != NONEVK && cache->visitKind != UTTFRMVK && cache->visitKind != UTTVK) {
        HError(9999, "GetCurUttElem: Function only support utterance based visiting");
    }
    if (cache->streamIdx != 1) {
        HError(9999, "GetCurUttElem: Lattices can only be visited through the cache of 1st stream");
    }
    ...
    *nSamples = 0;
    ...
    switch (cache->visitKind) {
        ...
        case PLUTTVK:
            ...
            break;
        default:
            HError(9999, "FillAllInpBatch: Unknown visiting order");
    }
    /* fill each input batches */
    for (i = 0; i < nInp; ++i) {
        batPtr = inpElem[i]->feaMat->matElems + (*nSamples) * inpElem[i]->extDim;
        if (inpElem[i]->inputKind == INPFEAIK) {
            GetDataFromCache(cache, inpElem[i], batPtr, 0);
        }
        else if (inpElem[i]->inputKind == AUGFEAIK) {	/* cz277 - aug */
            GetAugFeaFromCache(cache, inpElem[i], batPtr);
        }
        else {
            HError(9999, "FillAllInpBatch: Only INPFEAIK and AUGFEAIK could be loaded from disk");
        }
#ifdef CUDA
        SyncNMatrixHost2Dev(inpElem[i]->feaMat);
#endif
    }
    /* fill the label batch if needed */
    if (cache->labMat != NULL) {
        batPtr = cache->labMat->matElems + (*nSamples) * tgtDim;
        GetLabelFromCache(cache, tgtDim, batPtr, 0);
#ifdef CUDA
            ...
            if (m->type == 'h') {
                ...
                for (s = 2; s < hmm->numStates; ++s) {
                    ...
                    if (streamElem->occAcc <= 0.0) {
                        ...
                        if (mret == NULL) {
                            HError(9999, "UpdateTargetPen: HMM State %s[%d] has no training samples", m->id->name, s);
                        }
                        else {
                            HError(9999, "UpdateTargetPen: HMM State %s has no training samples", mret->id->name);
                        }
                    }
                }
            }
