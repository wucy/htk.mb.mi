   ...
   if ((posSt != NULL) || (tonSt != NULL)) {
      if ((posSt != NULL) && (matchMPEMask != NULL)) { 
         ...
         if (!(MaskMatch(matchMPEMask,mpeCntxt,cntxt))) {
            HError(1,"Can't match MPE mask with context");
         }       
...
static void SetBetaPlusCUDA() {
    ...
    if (fbInfo->aInfo->FBLatCUDA == FALSE) {
        HError(9999, "SetBetaPlusCUDA: The case is not valid for CUDA based FBLat rescore");
    }
...
/*chaopig*/
/*printf("UpTranParms: ta = %p, local_addindx = %d, add_index = %d, DoingFourthAcc = %d\n", ta, local_accindx, add_index, DoingFourthAcc);*/
   for(i=1;i<N;i++){
...
/*chaopig*/
/*printf("\ti = %d, j = %d, occ = %e, ti[j] = %e, occ[i] = %e\n", i, j, occ, ti[j], ta->occ[i]);*/
   ...
   for(s=1;s<=fbInfo->S;s++){
      ...
      for(m=0;m<nPDFs[s];m++){
         ...
         if(mp->ckind != INVDIAGC && mp->ckind != DIAGC){
            HError(-1, "HFBLat.c: expecting INVDIAGC or DIAGC");
         }
	...
	/* cz277 - ANN */
        ...
        if (fbInfo->aInfo->FBLatCUDA == FALSE) {
            if (fabs(total_occ) > 0.1) {
                HError(1, "in HFBLat.c: Wrong occ: exp(%f)\n", total_occ);
            }
...
Boolean FBLatFirstPass(FBLatInfo *_fbInfo, FileFormat dff, char * datafn, char *datafn2, Lattice *MPECorrLat){
    ...
    if (fbInfo->hsKind == HYBRIDHS) {	/* for hybrid systems, llhMat and T are already set */
        ...
        if (fbInfo->twoDataFiles) {
            if(!datafn2) {
                HError(1, "Need 2 data file names if single pass retraining.");
            }
            SetNewConfig("HPARM2");
...

    {
        ...
        for (q = 1; q <= fbInfo->Q; q++) {  
            ...
            if (ac->aclike == LZERO) {
                ...
                HError(-1, "Zero acoustic likelihood! (May be due to different model topology than used to create lattice)");
                FBLatClearUp(fbInfo);
		return FALSE;
            }
        }
        ...
        if (MPE) {  
            ...
            if (!MPECorrLat) {
                HError(-1, "No num lattice provided for MPE! ");        
            }
	        ...
	        if (CalcAsError) {
                ...
                if (nTrans1 != nTrans2) {
                    HError(-1, "NTrans1!=NTrans2"); /*maybe should be +1 error */
                }
