...
NVector *CreateNVector(MemHeap *x, int nlen)
{
    ...
    v->vecLen = nlen;
    size = NVectorElemSize(nlen);
#ifdef CUDA
    v->vecElems = (NFloat *) New(x, size);
    DevNew(&v->devElems, size);
#else
    #ifdef MKL
    v->vecElems = (NFloat *) mkl_malloc(size, MKL_alignment);
    #else
    v->vecElems = (NFloat *) New(x, size);
    #endif
#endif
...
NMatrix *CreateNMatrix(MemHeap *x, int nrows, int ncols)
{
    ...
    m->rowNum = nrows;
    m->colNum = ncols;
    size = NMatrixElemSize(nrows, ncols);
#ifdef CUDA
    m->matElems = (NFloat *) New(x, size);
    DevNew(&m->devElems, size);
#else
    #ifdef MKL
    m->matElems = (NFloat *) mkl_malloc(size, MKL_alignment);
    #else
    m->matElems = (NFloat *) New(x, size);
    #endif
#endif
...
void CopyVector2NVector(Vector v1, NVector *v2)
{
    ...
    size = VectorSize(v1);
    if (v2->vecLen != size) {
        HError(9999, "CopyVector2NVector: Inconsistent vector lengths");
    }
#ifdef CUDA
    for (i = 0; i < size; ++i)
...
void CopyNVector2Vector(NVector *v1, Vector v2) 
{
    ...
    size = VectorSize(v2);
    if (v1->vecLen != size) {
        HError(9999, "CopyNVector2Vector: Inconsistent vector lengths");
    }
#ifdef CUDA
...
void CopyMatrix2NMatrix(Matrix m1, NMatrix *m2)
{
    ...
    if (nrows != m2->rowNum || ncols != m2->colNum) {
        HError(9999, "CopyMatrix2NMatrix: Inconsistent matrices dimensions");
    }
#ifdef CUDA
    for (i = 0; i < nrows; ++i)
        for (j = 0; j < ncols; ++j)
...
void CopyNMatrix2Matrix(NMatrix *m1, Matrix m2) {
    ...
    if (nrows != m1->rowNum || ncols != m1->colNum) {
        HError(9999, "CopyNMatrix2Matrix: Inconsistent matrices dimensions");
    }
#ifdef CUDA
...
void CopyNMatrix2TrMatrix(NMatrix *m1, Matrix m2) {
    ...
    if (nrows != m1->colNum || ncols != m1->rowNum) {
        HError(9999, "CopyNMatrix2TrMatrix: Inconsistent matrices dimensions");
    }
#ifdef CUDA
    SyncNMatrixDev2Host(m1);
    for (i = 0; i < ncols; ++i)
...
void CopyMatrix2TrNMatrix(Matrix m1, NMatrix *m2) {
    ...
    if (nrows != m2->colNum || ncols != m2->rowNum) {
        HError(9999, "CopyMatrix2TrNMatrix: Inconsistent matrices dimensions");
    }
#ifdef CUDA
    for (i = 0; i < nrows; ++i)
        for (j = 0; j < ncols; ++j)
...
void CopyDMatrix2NMatrix(DMatrix m1, NMatrix *m2) {
    ...
    ncols = NumDCols(m1);
    if (nrows != m2->rowNum || ncols != m2->colNum) {
        HError(9999, "CopyDMatrix2NMatrix: Inconsistent matrices dimensions");
    }
    for (i = 0; i < nrows; ++i)
        for (j = 0; j < ncols; ++j)
            m2->matElems[i * m2->colNum + j] = m1[i + 1][j + 1];
#ifdef CUDA
    SyncNMatrixHost2Dev(m2);    
#endif
}
...
void CopyNMatrix2DMatrix(NMatrix *m1, DMatrix m2) {
    ...
    ncols = NumDCols(m2);
    if (nrows != m1->rowNum || ncols != m1->colNum) {
        HError(9999, "CopyNMatrix2DMatrix: Inconsistent matrices dimensions");
    }
#ifdef CUDA
     SyncNMatrixDev2Host(m1);
#endif
...
/*#ifdef CUDA
    SyncNMatrixDev2Host(m1);
    SyncNMatrixDev2Host(m2);
    for (i = 0; i < nrows; ++i)
        for (j = 0; j < ncols; ++j)
            m2->matElems[i * m2->colNum + j] = m1->matElems[i * m1->colNum + j];
    SyncNMatrixHost2Dev(m2);
#else
    for (i = 0; i < nrows; ++i)
        for (j = 0; j < ncols; ++j)
...
/*#ifdef CUDA
    SyncNVectorDev2Host(v1);
    SyncNVectorDev2Host(v2);
    for (i = 0; i < len; ++i)
        v2->vecElems[i] = v1->vecElems[i];
    SyncNVectorHost2Dev(v2);
#else
    for (i = 0; i < len; ++i)
        v2->vecElems[i] = v1->vecElems[i];
...
/*TODO:*/
void CopyNFloat2Float(NFloat *f1, float *f2) {
...
NMatrix *GenMaskTrNMatrix(MemHeap *x, int mappedTargetNum, IntVec mapVec) {
    ...
    for (i = 0; i < ncols; ++i) {
        /*mat->matElems[i * nrows + mapVec[i + 1]] = 1.0;*/
